...
This repository uses **jgehrcke/github-repo-stats** for long-term traffic tracking beyond GitHub's 14-day limit across **multiple repositories**.
...
The workflow uses `fail-fast: false` and `continue-on-error: true` so one failing repo doesn't stop the others.
